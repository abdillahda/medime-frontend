...
const MasterPeserta = () => {
  ...
  const handleChange = (e) => {
    ...
    if (name === "uidCompany") {
      ...
      setFormData((prev) => ({ ...prev, [name]: value, packageUid: "" }));
    }
  };
  ...
  const handleOpenUpdate = (p) => {
    ...
    setFormData({
      ...
      registrationRemarks: p.registrationRemarks || "",
    });

    if (p.companyUid) fetchPackagesByCompany(p.companyUid);
  ...
  const handleSubmit = async (e) => {
    ...
    if (!formData.phoneNumber?.toString().trim())
      missingFields.push("No. Telepon/HP");
    if (!formData.uidCompany) missingFields.push("Perusahaan");
    if (!formData.packageUid) missingFields.push("Paket MCU");
    ...
    try {
      ...
      setNotification({
        ...
        message: `Gagal memproses data ke server.`,
      });
    }
  };
  ...
  const handleToggleStatus = async (p) => {
    const nextStatus =
      p.status === "AKTIF" || p.status === "Aktif" ? "NONAKTIF" : "AKTIF";
    try {
      ...
      <header className="p-8 pb-4 flex justify-between items-center bg-white border-b border-gray-100">
        <div>
          ...
          <nav className="text-xs text-gray-400 mt-1 italic">
            Manajemen Database &gt;{" "}
            <span className="text-cyan-700 font-bold">Daftar Peserta</span>
          </nav>
        </div>
              ...
              {pesertaList.map((p) => (
                <tr
                  ...
                  className={`group transition-all ${
                    p.status === "Tidak Aktif" || p.status === "TIDAK_AKTIF"
                ...
                >
                  ...
                  <td className="p-5">
                    ...
                    <div className="text-[10px] text-gray-400 uppercase ml-4">
                      {p.positionName || "-"}
                    </div>
                  </td>
                  <td className="p-5">
                        ...
                        <div className="absolute right-14 top-1/2 -translate-y-1/2 w-48 bg-white rounded-xl shadow-[0_5px_25px_-5px_rgba(0,0,0,0.1)] border border-gray-100 z-20 py-2 overflow-hidden animate-in fade-in zoom-in-95 duration-200">
                          {p.status === "AKTIF" || p.status === "Aktif" ? (
                            <button
                              ...
                              className="w-full text-left px-4 py-2.5 text-[13px] font-semibold text-red-500 hover:bg-red-50 flex items-center gap-3 transition-colors"
                            >
                            ...
                            <button
                              onClick={() => handleToggleStatus(p)}
                              className="w-full text-left px-4 py-2.5 text-[13px] font-semibold text-green-600 hover:bg-green-50 flex items-center gap-3 transition-colors"
                            >
        ...
        <div className="p-5 border-t border-gray-50 flex items-center justify-between bg-gray-50/30">
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className="p-2 text-gray-400 hover:text-cyan-700 disabled:opacity-30 transition-all"
            >
              <ChevronLeft size={20} />
            </button>
        ...
        <>
          ...
          <div className="fixed right-0 top-0 h-full w-[500px] bg-white shadow-2xl z-50 p-0 flex flex-col animate-in slide-in-from-right duration-300">
            ...
            <form className="p-8 space-y-5 overflow-y-auto flex-1 scrollbar-hide">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="flex flex-col space-y-1">
                  ...
                  <input
                    ...
                    onChange={handleChange}
                    type="date"
                    className="w-full p-3 border border-gray-200 rounded-xl text-sm text-gray-500 outline-none"
                  />
                </div>
                <div className="flex flex-col space-y-1">
                  <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest ml-1">
              ...
              <div className="flex flex-col space-y-4 pt-4 border-t border-dashed">
                <div className="flex flex-col space-y-1">
                  ...
                  <select
                    ...
                    onChange={handleChange}
                    className="w-full p-3 bg-white border border-gray-200 rounded-xl text-sm focus:ring-2 focus:ring-cyan-500 outline-none"
                  >
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div className="flex flex-col space-y-1">
                    ...
                    <input
                      name="companyDepartmentPosition"
                      value={formData.companyDepartmentPosition}
                      onChange={handleChange}
                      type="text"
              ...
              {/* --- BAGIAN PAKET (TAMPIL DI KEDUA MODE) --- */}
              <div className="flex flex-col space-y-4 pt-4 border-t border-dashed bg-cyan-50/30 -mx-8 px-8 pb-5">
                <div className="flex flex-col space-y-1">
                  ...
                  <select
                    name="packageUid"
                    value={formData.packageUid}
                    onChange={handleChange}
                    disabled={!formData.uidCompany || isPackageLoading}
                    className="w-full p-3 bg-white border border-cyan-100 rounded-xl text-sm focus:ring-2 focus:ring-cyan-500 outline-none disabled:opacity-50"
                  >
                    <option value="">
                      {isPackageLoading
                        ? "Memuat Paket..."
                  ...
                  {drawerMode === "update" && (
                    <p className="text-[9px] text-cyan-600 mt-1 italic font-medium ml-1">
                      * Pilih paket untuk sekaligus mendaftarkan ulang / update
                      layanan MCU peserta.
                    </p>
                  )}
                </div>
                <div className="flex flex-col space-y-1">
                  ...
                  <textarea
                    ...
                    onChange={handleChange}
                    className="w-full p-3 bg-white border border-cyan-100 rounded-xl text-sm outline-none"
            ...
            <div className="p-6 overflow-y-auto flex-1">
              {isDetailLoading ? (
                ...
                <div className="space-y-6">
                  <div>
                    <h3 className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-3 pb-2 border-b border-dashed text-left">
                      Identitas Pasien
                    </h3>
                    <div className="grid grid-cols-2 gap-y-4 gap-x-6 text-left">
                      ...
                      <div>
                        ...
                        <p className="text-sm font-medium text-gray-800">
                          {selectedPeserta.gender} • {selectedPeserta.age} Tahun
                        </p>
                      </div>
                      <div>
                        <p className="text-[10px] text-gray-400 uppercase font-bold mb-1">
                        ...
                        <div className="flex items-center text-sm font-medium text-gray-800">
                          ...
                          />{" "}
                          {selectedPeserta.birthdayDate}
                        </div>
                      </div>
                    </div>
                  </div>
                  <div className="bg-cyan-50/50 p-4 rounded-xl border border-cyan-100/50 text-left">
                    <h3 className="text-[10px] font-bold text-cyan-800 uppercase tracking-widest mb-3 text-left">
                      Informasi Kemitraan (B2B)
                    </h3>
                    <div className="grid grid-cols-2 gap-y-3 gap-x-6 text-left">
                      <div className="col-span-2">
                        <p className="text-[10px] text-cyan-600/70 uppercase font-bold mb-1">
                          Perusahaan
                        </p>
                        <div className="flex items-center text-sm font-bold text-cyan-800 uppercase">
                          <Building2 size={14} className="mr-1.5" />{" "}
                          {selectedPeserta.companyName || "-"}
                        </div>
                      </div>
                      <div>
                        <p className="text-[10px] text-cyan-600/70 uppercase font-bold mb-1">
                        ...
                        <p className="text-sm font-semibold text-cyan-900 uppercase">
                          {selectedPeserta.departmentName || "-"}
                        </p>
                      </div>
                      <div>
                        <p className="text-[10px] text-cyan-600/70 uppercase font-bold mb-1">
